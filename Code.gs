...
function doGet(e) {
  ...
  } else if (e.parameter.action === 'getVotos') {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName('Votação');
    if (!sheet) {
      result = { success: false, error: 'Aba Votação não encontrada' };
    } else {
      const rows = sheet.getDataRange().getValues();
      const votos = [];
      for (let i = 1; i < rows.length; i++) {
        const r = rows[i];
        if (r[3]) {
          votos.push({ candidatoVotado: r[3], numeroCandidato: r[4] });
        }
      }
      result = { success: true, data: votos };
    }
...
function doPost(e) {
  ...
  try {
    ...
    if (data.action === 'submitInscricao') {
      ...
      const sheet = ss.getSheetByName('Votação');
      sheet.appendRow([
        new Date(), data.nomeVotante, '',
        data.candidatoVotado, data.numeroCandidato
      ]);
      result = { success: true, message: 'Voto registrado com sucesso!' };

    } else if (data.action === 'setCandidates') {
      const sheet = ss.getSheetByName('candidatos');
      if (!sheet) {
        result = { success: false, error: 'Aba candidatos não encontrada' };
      } else {
        // Mantém cabeçalho e substitui os dados
        const lastRow = sheet.getLastRow();
        if (lastRow > 1) sheet.getRange(2, 1, lastRow - 1, 2).clearContent();
        const nomes = (data.nomes || []).filter(n => n && n.trim());
        if (nomes.length > 0) {
          const rows = nomes.map((n, i) => [i + 1, n.trim()]);
          sheet.getRange(2, 1, rows.length, 2).setValues(rows);
        }
        result = { success: true, message: 'Candidatos atualizados!' };
      }
